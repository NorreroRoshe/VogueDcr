...
export const OrderDetailsContent: React.FC<{ item?: any }> = ({ item }) => {
  ...
  const { price } = usePrice({
    ...
    const photoUrlBase = process.env.NEXT_PUBLIC_PHOTO_URL;
  ...
  return (
    <div className="relative grid grid-cols-12 py-2 pb-0 border-b border-solid border-skin-base text-[12px] md:text-[14px]">
      ...
      {item.image &&
        <img
          src={`${photoUrlBase}storage/${item.image}`}
      ...
      <div className="col-span-5 self-center">
        <h2 className="text-skin-base" style={{fontSize: '14px', marginLeft: '5px'}}>{item?.name} {item?.article}</h2>
      </div>
