...
const AfterVerification: React.FC<VerificationProps> = ({ isPopup = true, className }) => {
        ...
        <div className="w-full md:w-[50%] xl:w-[50%] py-6 sm:py-10 px-4 sm:px-8 xl:px-12 rounded-md shadow-dropDown flex flex-col justify-center">
          <div className="text-center mb-6 ">
            ...
            <div className="text-sm sm:text-15px text-body text-center mt-3 mb-1">
              {t('Поздравляем, вы успешно верифицировали аккаунт! Теперь вы можете пользоваться всеми функциями нашего сайта :)')}
              <br />
            </div>
            
            <Link href='/' style={{borderRadius: '15px', border: '1px solid #0085FF', padding: '5px 15px', margin: '30px auto 0px', display: 'block', width: 'fit-content'}} className="text-skin-base font-semibold text-xl sm:text-2xl sm:pt-3 ">
              {t('На главную')}
            </ Link>
          </div>
        </div>
